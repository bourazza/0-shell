pub fn run() -> Result<(), String> {
    ...
    let commands = [
        (
            "echo [-n] [args...]",
            "Print text to stdout. -n suppresses trailing newline.",
        ),
        ...
        (
            "cat [-n] [file...]",
            "Concatenate and print files. -n shows line numbers.",
        ),
        (
            "cp [-r] src... dest",
        ...
        (
            "rm [-r] [-f] file...",
            "Remove files or directories. -r recursive, -f force.",
        ),
        (
            "mkdir [-p] dir...",
            "Create directories. -p creates parent dirs as needed.",
        ),
        ("exit", "Exit the shell."),
    ...
    println!(
        "  Command chaining with \x1b[1m;\x1b[0m  (e.g. \x1b[36mmkdir foo; cd foo; pwd\x1b[0m)"
    );
